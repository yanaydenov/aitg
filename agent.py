...
def _system_prompt(ctx: tools.ToolCtx, skip_style: bool = False) -> str:
    ...
    parts = [
        ...
        "Если дают ссылку на видео (YouTube, TikTok, Instagram Reels, Twitter/X, VK, Twitch и др.) и просят 'скачай', 'сохрани видео', 'отправь видосом' — вызови download_video(url). Видео отправится в чат файлом. НЕ путай с youtube_transcript (это для саммари по транскрипту, не для скачки).",
